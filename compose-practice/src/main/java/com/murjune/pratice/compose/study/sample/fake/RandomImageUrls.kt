...
@Composable
fun rememberRandomSamplePhotos(count: Int): List<Photo> =
    remember {
        List(count) {
...
@Composable
fun rememberRandomSampleImageUrls(count: Int): List<String> =
    remember(count) {
        List(count) {
 ...
 * - 작은 이미지 (300x400)
 */
private val randomSizedImageUrls =
    listOf(
...
private val randomSamplePhotos: List<Photo> =
    listOf(
        Photo(
